...
def get_parser():
    ...
    parser.add_argument(
        '--test-batch-size', type=int, default=10, help='test batch size')
    parser.add_argument(
        '--start-epoch',
        type=int,
        default=0,
...
class Processor():
    ...
    def load_model(self):
        ...
        if self.arg.weights:
            ...
            weights = OrderedDict(
                [[k.split('module.')[-1],
                  v] for k, v in weights.items()])  # v.cuda(output_device)
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            self.global_step += 1
            # get data
            data = Variable(data.float(), requires_grad=False).cuda(self.output_device)  # '''.cuda(self.output_device)'''
            label = Variable(label.long(), requires_grad=False).cuda(self.output_device)  # '''.cuda(self.output_device)'''
        ...
        if save_model:
            ...
            torch.save(weights, self.arg.model_saved_name + '-' + str(epoch) + '-' + str(int(self.global_step)) + '.pth')
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    data = Variable(
                        data.float().cuda(self.output_device),  # '''.cuda(self.output_device)''',
                        requires_grad=False,
                        volatile=True)
                    label = Variable(
                        label.long().cuda(self.output_device),  # '''.cuda(self.output_device)''',
                        requires_grad=False,
                        volatile=True)
                    output = self.model(data)

                    # t = temp.cpu().detach().numpy()
                    # np.save('./data/m2d/motion_net_test.npy', t)
            ...
            for i in range(len(self.data_loader[ln].dataset.sample_name)):
