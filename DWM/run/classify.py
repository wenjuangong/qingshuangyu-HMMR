...
if __name__ == '__main__':
    ...
    for i in range(len(data)):
        ...
        if data[i].split('_')[1] == 'C':
            ori_label.append(0)
        elif data[i].split('_')[1] == 'R':
            ori_label.append(1)
        elif data[i].split('_')[1] == 'T':
            ori_label.append(2)
        else:
            ori_label.append(3)

        # num_seq = len(temp_data) // seq_len
        # for m in range(num_seq-1):
        #     for n in range(0,seq_len,5):
        #         if data[i].split('_')[1] == 'C':
        #             ori_label.append(0)
        #         elif data[i].split('_')[1] == 'R':
        #             ori_label.append(1)
        ...
        #         else:
        #             ori_label.append(3)
        #         indices = [k for k in range(m*seq_len+n,m*seq_len+seq_len+n)]
        #         ori_data = temp_data[np.newaxis, indices] if ori_data is None else np.vstack((ori_data, temp_data[np.newaxis ,indices]))
    pre_data = []
    pre_label = []
    for i in range(len(test_dirs)):
        ...
        if test_dirs[i].split('_')[-2] == 'C':
            pre_label.append(0)
        elif test_dirs[i].split('_')[-2] == 'R':
            pre_label.append(1)
        elif test_dirs[i].split('_')[-2] == 'T':
            pre_label.append(2)
        else:
            pre_label.append(3)
        ...
        #             pre_label.append(3)
        #         indices = [k for k in range(m * seq_len + n, m * seq_len + seq_len + n)]
        #         pre_data = temp_data[np.newaxis, indices] if pre_data is None else np.vstack(
        #             (pre_data, temp_data[np.newaxis, indices]))
    pre = []
